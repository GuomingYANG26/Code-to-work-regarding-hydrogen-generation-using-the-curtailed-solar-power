# -*- coding: utf-8 -*-
"""
Created on Wed Jul 19 19:35:04 2023
...
def curtailment_multiplier (c_Elz):
    #光伏出力
...
number_Elz = 106  #Elz_price在50-1100之间划分的数量
cost_Elz = np.linspace(50, 1100, number_Elz)
obj_0 = np.zeros(number_Elz)
P_Elz_value_0 = np.zeros(number_Elz)
N_BS_value_0 = np.zeros(number_Elz)
P_PV_value_0 = np.zeros(number_Elz)
Firm_kWh_Premium_0 = np.zeros(number_Elz)
Firm_LCOH_0 = np.zeros(number_Elz)
for i in range(number_Elz):
    obj_0[i],  P_Elz_value_0[i], N_BS_value_0[i], P_PV_value_0[i], Firm_kWh_Premium_0[i], Firm_LCOH_0[i] = curtailment_multiplier (cost_Elz[i])
    print('-------------------------休息会----------------------------')
    time.sleep(0.2)
    print(cost_Elz[i])
#保存数据
workbook = xlsxwriter.Workbook('D:/Doctor/paper/2023third/Last_Code/5.4/potential.xlsx')
worksheet = workbook.add_worksheet()
#写入行名
row = 0
col = 0
worksheet.write(row, col, 'c_Elz ($/kW)')
worksheet.write(row, col+1, 'obj ($)')
worksheet.write(row, col+2, 'P_Elz_value (kW)')
worksheet.write(row, col+3, 'N_BS_value (MWh)')
worksheet.write(row, col+4, 'P_PV_value (MW)')
worksheet.write(row, col+5, 'Firm_kWh_Premium (dimensionless)')
worksheet.write(row, col+6, 'Firm_LCOH ($/kWh)')
row += 1
#一行行写入数据
buffer = 0
while buffer < number_Elz :
    worksheet.write(row, col, cost_Elz[buffer])
    worksheet.write(row, col+1, obj_0[buffer])
    worksheet.write(row, col+2, P_Elz_value_0[buffer])
    worksheet.write(row, col+3, N_BS_value_0[buffer])
    worksheet.write(row, col+4, P_PV_value_0[buffer])
    worksheet.write(row, col+5, Firm_kWh_Premium_0[buffer])
    worksheet.write(row, col+6, Firm_LCOH_0[buffer])
    row += 1
    buffer += 1
workbook.close()
